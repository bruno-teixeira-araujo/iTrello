...
@Component
public class DashboardDueDateByBoardlistCalculator implements DashboardCalculator {

	@Override
	public List<DashboardElement> calculateBoardInfo(List<Boardlist> boardlists, List<Card> cards) {
		Map<String, DashboardElement> dashboard = initDashboardMap(boardlists);
		...
		for(Card card : cards) {
			...
			element.incNrDashboardTotalElems();
		}
	...
	private Map<String, DashboardElement> initDashboardMap(List<Boardlist> boardlists) {
		Map<String, DashboardElement> dashboard = new LinkedHashMap<>();
		for(Boardlist boardlist : boardlists) {
			dashboard.put(boardlist.getId(), new DashboardElement(boardlist.getId(), boardlist.getName(), 0L));
		}
		return dashboard;
	}
}
